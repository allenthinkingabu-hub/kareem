...
*   **Task 1.1: 拆解提取大盘商业与功能目标**：读取系统主架构师汇总的全局看板，将其转化为专属于你这个后端的具体业务场景（Use Cases）和并发应对目标。
*   **Task 1.2: 映射非功能性约束 (NFR) 与异步追问**：从看板的红线说明中提取你的库表一致性约定及安全合规极值（如 QPS/TPS 延时底限）。若指标缺失，必须写入看板底部的【待解决疑问清单】等待打包解答。
